...
class ShoppingCart
{
    static function addNewOrder($chat_id, $productId, $count, $optionNum) {
        ...
        if (isset($arr['price'])) {
            $productId = $db->real_escape_string($arr['id']);
            ...
            if (mysqli_num_rows($resultShoppingCart) == 0) {
                $resultInsert = $db->query("INSERT INTO `shoppingcart` (`userId`, `productId`, `quantity`, `optionNum`) VALUES ('{$chat_id}', '{$productId}', '{$count}', '{$optionNum}')");
            } else {
                $arr = $resultShoppingCart->fetch_assoc();
                $id = $arr['id'];
                $oldQuantity = $arr['quantity'];
                $newQuantity = (int) $oldQuantity + (int) $count;
                $resultUpdate = $db->query("UPDATE `shoppingcart` SET `quantity` = '$newQuantity' WHERE `shoppingcart`.`id` = '$id'");
            }
        }
    }
