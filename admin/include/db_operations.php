...
class Operation extends Database
{
    ...
    public function selectbystatus($table, $status)
    {
    ...
    public function telegramsend()
    {
        ...
        send1($title, $body, "https://storage.kun.uz/source/thumbnails/_medium/6/2PKczBR1dVBy1tlpAz4ZKlt3LAytrc0d_medium.jpg", "https://blogcamp.uz/blog/" . $id);
    ...
    public function gmailsend()
    {
        ...
        while ($rows = $stmt->fetch()) {
            $to = $rows["mail"];
            gmails($to, $title, "https://storage.kun.uz/source/thumbnails/_medium/6/2PKczBR1dVBy1tlpAz4ZKlt3LAytrc0d_medium.jpg", $body, "https://blogcamp.uz/blog/" . $id);
        }
    ...
    {
        $sql = "SELECT photoUrl FROM " . $table . " WHERE id={$id}";
        $result = $this->db->query($sql);
        while ($rows = $result->fetch()) {
            $imagename = $rows['photoUrl'];
        }
        $deleteimage = "../photos/" . $imagename;
